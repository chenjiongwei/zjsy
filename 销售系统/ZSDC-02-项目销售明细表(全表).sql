...
GO
/****** Object:  StoredProcedure [dbo].[usp_rpt_s_ProjSaleDetailsInfo]    Script Date: 2024/12/4 15:21:47 ******/
SET ANSI_NULLS ON
GO
...
SELECT 
            ...
            /*财务信息*/
            t1.AjfkDate, --按揭放款日期
            t1.GjjfkDate, --公积金放款日期
            ISNULL(t1.ssdj, 0) AS SsDj, --实收定金
			t1.ssdjDate AS ssdjDate, --实收定金日期
            ISNULL(t1.sssf, 0) AS Sssf, --实收首付款
            ISNULL(t2.ssdkfk, 0) AS Ssdkfk, --实收贷款类房款
            ISNULL(t2.ssfdkfk, 0) AS Ssfdkfk, --实收非贷款类房款
            ISNULL(t1.ssBcxyk, 0) AS SsBcxyk, --实收补充协议款
            ISNULL(t1.ssDsfy, 0) AS SsDsfy, --实收代收费用款
            ISNULL(t2.ssAj, 0) AS SsAj, --实收银行按揭款
            ISNULL(t2.ssGjj, 0) AS SsGjj, --实收公积金款
            ISNULL(t2.sslk, 0) AS Sslk, --实收楼款
            ISNULL(t2.ssbc, 0) AS Ssbc --实收补差款
  FROM      (    SELECT WideGUID,
                           ...
                           SELECT WideGUID,
                                  TradeGUID
                             FROM s_Contract) sale
  LEFT JOIN (   SELECT SUM(CASE
                                WHEN ItemNameGUID = 'EB3911EF-83DF-4486-8DB1-19E49AF7D8A0' THEN s_Getin.BeforeRmbYe
                                ELSE NULL END) ssdj, /*实收定金*/
					   MAX(CASE WHEN s_Getin.ItemNameGUID = 'EB3911EF-83DF-4486-8DB1-19E49AF7D8A0' 
                            then  case when   v.VouchType = '转账单' THEN  v.skDate ELSE  s_Getin.GetDate end  else  null   END ) AS ssdjDate, /*实收定金日期*/
                       SUM(CASE
                                WHEN ItemNameGUID = 'C5DD4426-82D9-46EB-8521-9DC3C5515461' THEN s_Getin.BeforeRmbYe
                                ELSE NULL END) sssf, /*实收首付款*/
                       MIN(CASE
                                WHEN ItemNameGUID = '9165FAED-227A-465D-AA5D-D24BED655677' THEN s_Getin.GetDate
                                ELSE NULL END) AjfkDate, /*按揭放款日期*/
                       MIN(CASE
                                WHEN ItemNameGUID = 'C3190DC3-C295-4A98-B7AC-9DFF7D7A0091' THEN s_Getin.GetDate
                                ELSE NULL END) GjjfkDate, /*公积金放款日期*/
                       SUM(CASE
                                WHEN ItemTypeGUID = '394AAB92-81B8-46EC-8A55-34790A126167' THEN s_Getin.BeforeRmbYe
                                ELSE NULL END) ssBcxyk, /*实收补充协议款*/
                       SUM(CASE
                                WHEN ItemTypeGUID = '58EEBFEA-E24D-4ADB-84FB-C5E30CAE71FB'
                                 AND dbo.s_Getin.StatusEnum <> 1 /*作废*/ /*划出*/ THEN s_Getin.BeforeRmbYe
                                ELSE NULL END) ssDsfy, /*实收代收费用*/
                       s_Getin.SaleGUID
                  FROM s_Getin 
                  inner join s_voucher v on v.VouchGUID = s_Getin.VouchGUID
                 WHERE s_Getin.StatusEnum <> 2
                 GROUP BY s_Getin.SaleGUID) t1 ON t1.SaleGUID  = sale.TradeGUID
  LEFT JOIN (   SELECT      SUM(CASE
                                     WHEN ItemTypeGUID = '4D402C5B-DE7B-437E-956F-8178DA46BBF2'
                                      AND IsFk = 1 THEN s_Getin.BeforeRmbYe
                                     ELSE NULL END) ssdkfk, /*实收贷款类房款*/
                            SUM(CASE
                                     WHEN ItemTypeGUID = '716D2B9D-E1CF-4039-8DBE-8CB01CECA6FF'
                                      AND IsFk = 1 THEN s_Getin.BeforeRmbYe
                                     ELSE NULL END) ssfdkfk, /*实收非贷款类房款*/
                            SUM(CASE
                                     WHEN ItemNameGUID = 'C3190DC3-C295-4A98-B7AC-9DFF7D7A0091'
                                      AND IsFk = 1 THEN s_Getin.BeforeRmbYe
                                     ELSE NULL END) ssGjj, /*实收公积金*/
                            SUM(CASE
                                     WHEN ItemNameGUID = '9165FAED-227A-465D-AA5D-D24BED655677'
                                      AND IsFk = 1 THEN s_Getin.BeforeRmbYe
                                     ELSE NULL END) ssAj, /*实收银行按揭*/
                            SUM(CASE
                                     WHEN ItemNameGUID = 'F72367DE-91CD-484E-8E2B-FEEA8E6C0DDA'
                                      AND IsFk = 1 THEN s_Getin.BeforeRmbYe
                                     ELSE NULL END) sslk, /*实收楼款*/
                            SUM(CASE
                                     WHEN ItemNameGUID = '44234C3E-CABC-4534-A603-DB4776A47D6F' THEN s_Getin.BeforeRmbYe
                                     ELSE NULL END) ssbc, /*实收补差款*/
                            s_Getin.SaleGUID
                  FROM    s_Getin
                  LEFT JOIN s_FeeItem ON s_Getin.ItemNameGUID = s_FeeItem.FeeItemGUID
                 WHERE      s_Getin.StatusEnum <> 2 /*作废*/
                 GROUP BY s_Getin.SaleGUID) t2
    ON t2.SaleGUID  = sale.TradeGUID
