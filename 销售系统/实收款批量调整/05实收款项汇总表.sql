...
SUM(case when year(sg.skdate)=@年份 AND  sg.itemtype IN ('非贷款类房款','贷款类房款','补充协议款') then sg.RmbAmount end) AS 业绩口径本年实收,
sum(case when year(sg.skdate)=@年份 then sg.RmbAmount end) as 本年累计实收,
sum(case when year(sg.skdate)=@年份 and month(sg.skdate)=1 then sg.RmbAmount end) as 一月累计实收,
sum(case when year(sg.skdate)=@年份 and month(sg.skdate)=2 then sg.RmbAmount end) as 二月累计实收,
sum(case when year(sg.skdate)=@年份 and month(sg.skdate)=3 then sg.RmbAmount end) as 三月累计实收,
sum(case when year(sg.skdate)=@年份 and month(sg.skdate)=4 then sg.RmbAmount end) as 四月累计实收,
sum(case when year(sg.skdate)=@年份 and month(sg.skdate)=5 then sg.RmbAmount end) as 五月累计实收,
sum(case when year(sg.skdate)=@年份 and month(sg.skdate)=6 then sg.RmbAmount end) as 六月累计实收,
sum(case when year(sg.skdate)=@年份 and month(sg.skdate)=7 then sg.RmbAmount end) as 七月累计实收,
sum(case when year(sg.skdate)=@年份 and month(sg.skdate)=8 then sg.RmbAmount end) as 八月累计实收,
sum(case when year(sg.skdate)=@年份 and month(sg.skdate)=9 then sg.RmbAmount end) as 九月累计实收,
sum(case when year(sg.skdate)=@年份 and month(sg.skdate)=10 then sg.RmbAmount end) as 十月累计实收,
sum(case when year(sg.skdate)=@年份 and month(sg.skdate)=11 then sg.RmbAmount end) as 十一月累计实收,
sum(case when year(sg.skdate)=@年份 and month(sg.skdate)=12 then sg.RmbAmount end) as 十二月累计实收
...
--and year(sf.lastdate)=@年份
--and year(sg.skdate)=@年份
group by 
pro.projname,
pro.SpreadName,
fq.projshortname,
