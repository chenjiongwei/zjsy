...
(
    ...
    where sg.itemtype in ('贷款类房款','非贷款类房款','补充协议款') and isnull(sg.vouchstatus,'')<>'作废' 
    AND  sg.VouchType NOT IN ( 'POS机单', '划拨单', '放款单' )
    and sg.cwskdate<=@date
    group by sg.saleguid
) tt

-- 创建临时表存储认购情况(往年、本年、全年)
...
(
    ...
    where year(so.ZcOrderDate)<year(@date) and so.ZcOrderDate<=@date  and (so.ostatus='激活' or so.cstatus='激活')
    group by 
        so.projguid,
        bld.ProductTypeName
    ...
    left join #应收情况 sf on so.tradeguid=sf.tradeguid
    left join #实收情况 sg on so.tradeguid=sg.saleguid
    where so.ZcOrderDate<=@date  and (so.ostatus='激活' or so.cstatus='激活')
    group by 
        so.projguid,
        bld.ProductTypeName
) tt

-- 创建临时表存储签约情况(往年、本年、全年)
...
-- 清理临时表
drop table #认购情况,#签约情况,#应收情况,#实收情况,#草签情况