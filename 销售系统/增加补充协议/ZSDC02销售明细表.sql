...
GO
/****** Object:  StoredProcedure [dbo].[usp_rpt_s_ProjSaleDetailsInfo]    Script Date: 2025/5/22 17:25:46 ******/
SET ANSI_NULLS ON
GO
...
BEGIN
    SELECT p.p_projectId,
           ...
           ISNULL(r.CjRmbTotal, 0) -isnull(bx.RmbAmount,0) as 房间总价不含补充协议,
           ...
           CASE
               WHEN ( ISNULL(tr.Ysfk, 0) + ISNULL(tr.BcTotal, 0) ) = 0 THEN
                   0
               ELSE
                   ISNULL(g.getRmbAmount, 0) / (ISNULL(tr.Ysfk, 0) + ISNULL(tr.BcTotal, 0) )
           END AS 累计收款比例,
           ...
           ISNULL(r.CjRmbTotal, 0) + ISNULL(tr.BcTotal, 0) AS 面积补差后实际成交金额,
