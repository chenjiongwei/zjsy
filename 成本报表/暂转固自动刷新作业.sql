...
*/
CREATE PROC [dbo].[usp_cb_zzg_auto_refresh]
AS
BEGIN
    -- 查询计划系统中包含"暂转固"字眼的计划工作项获取最新的实际完成日期插入临时表
    SELECT 
        proj.p_projectId,
        CASE 
    ...
    WHERE pte.TaskName = '确定总包确认价(暂转固)'
        AND pte.ActualFinishTime IS NOT NULL 
        AND pte.PlanType = 1
    ...
    FROM cb_Contract a 
    INNER JOIN cb_ContractProj cp 
        ON cp.ContractGUID = a.ContractGUID
    INNER JOIN cb_ContractExtend ce 
        ON ce.ContractGUID = a.ContractGUID
    WHERE a.IsNeedBg = 1 
        AND a.IsOverallContract = 1  
        AND ce.ActualBgTime IS NULL

    --更新成本系统中暂转固合同的实际转固日期
    UPDATE ce  
    SET ce.ActualBgTime = tpt.ActualFinishTime
    FROM cb_ContractExtend ce
    INNER JOIN #tmp_zzg_contract tzc 
        ON tzc.ContractGUID = ce.ContractGUID
    OUTER APPLY (
        SELECT TOP 1 
            tpt.PeriodGUID projguid,  
            ActualFinishTime 
        FROM #tmp_zzg_plan_task tpt 
        WHERE tpt.PeriodGUID = tzc.ProjGUID 
        ORDER BY tpt.ActualFinishTime DESC
    ) tpt
    WHERE ce.ActualBgTime IS NULL 
        AND tpt.ActualFinishTime IS NOT NULL
